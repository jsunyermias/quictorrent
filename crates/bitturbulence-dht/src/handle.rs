...
use crate::{error::Result, message::DhtMessage, node::DhtNode, node_id::NodeId};
        ...
        loop {
            match tokio::time::timeout_at(deadline, rx.recv()).await {
                Ok(Ok((
                    ...
                    for node in n {
                        self.node.add_node(node.id, node.addr.clone());
                    }
                }
